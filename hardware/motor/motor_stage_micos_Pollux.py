...
class MotorStageMicosPollux(Base, MotorInterface):
    """unstable: Jochen Scheuer.
    Hardware class to define the controls for the Micos stage of PI.
    """
    _modclass = 'MotorStageMicosPollux'
    _modtype = 'hardware'

    unit_factor = 1000. # This factor converts the values given in m to mm.
    ### !!!!! Attention the units can be changed by setunit

    _com_port_xyzphi = ConfigOption('com_port_xyzphi', 'COM6', missing='warn')
    _baud_rate_xyzphi = ConfigOption('baud_rate_xyzphi', 19200, missing='warn')
    _timeout_xyzphi = ConfigOption('timeout_xyzphi', 1000, missing='warn')
    _term_char_xyzphi = ConfigOption('term_char_xyzphi', '\r', missing='warn')
    ...
    _min_fourth = ConfigOption('fourth_min', 0.0, missing='warn')
    _max_fourth = ConfigOption('fourth_max', 360.0, missing='warn')
    ...
    def on_activate(self):
        ...
        self._serial_connection_xyzphi = self.rm.open_resource(
            resource_name=self._com_port_xyzphi,
            baud_rate=self._baud_rate_xyzphi,
            timeout=self._timeout_xyzphi)
...
#                                                           ))
#
        self._write('z', '{} {} 3 setnlimit'.format(constraints['z']['pos_min'] * self.unit_factor,
    ...
    def on_deactivate(self):
        ...
        @return: error code
        """
        self._serial_connection_xyzphi.close()
        self.rm.close()
        return 0
        ...
        for attept in range(3):
            try:
                ...
                if 'x' in param_dict and 'y' in param_dict:
                    self._write('x', '{} 1 nm;{} 2 nm'.format(param_dict['x']*self.unit_factor,
                    ...
                    if 'y' in param_dict:
                        self._write('y', '{} 1 nm;{} 2 nm'.format(curr_pos['x']*self.unit_factor,
                                                               param_dict['y']*self.unit_factor))
                ...
                if 'z' in param_dict and 'phi' in param_dict:
                    self._write('z', '{} 3 nm;{} 4 nm'.format(param_dict['z']*self.unit_factor,
                    ...
                    if 'phi' in param_dict:
                        self._write('phi', '{} 3 nm;{} 4 nm'.format(curr_pos['z']*self.unit_factor,
                                                                 param_dict['phi']*self.unit_factor))
        ...
        try:
            if param_list is not None:
                for axis_label in param_list:
                    ...
                    if axis_label == 'x':
                        param_dict[axis_label] = self._ask(axis_label, '1 nst')
                    if axis_label == 'y':
                        param_dict[axis_label] = self._ask(axis_label, '2 nst')
                    if axis_label == 'z':
                        param_dict[axis_label] = self._ask(axis_label, '3 nst')
                    if axis_label == 'phi':
                        param_dict[axis_label] = self._ask(axis_label, '4 nst')
            else:
                for axis_label in constraints:
                    #the status check takes quite long so if port is checked
                    # there is no need for second check
                    if constraints[axis_label]['label'] == 'x':
                        param_dict[axis_label] = self._ask(axis_label, '1 nst')
                    if constraints[axis_label]['label'] == 'y':
                        param_dict[axis_label] = self._ask(axis_label, '2 nst')
                    if constraints[axis_label]['label'] == 'z':
                        param_dict[axis_label] = self._ask(axis_label, '3 nst')
                    if constraints[axis_label]['label'] == 'phi':
                        param_dict[axis_label] = self._ask(axis_label, '4 nst')
            return param_dict
        except:
        ...
        if param_list is not None:
            for axis_label in constraints:
                ...
                if constraints[axis_label]['label'] == 'z' in param_list:
                    self._write('z', '3 ncal')

                if constraints[axis_label]['label'] == 'phi' in param_list:
                    self._write('phi', '4 ncal')

        else:
            # setting axes active
            self._write('x','1 ncal;2 ncal;3 ncal;4 ncal')
    ...
    def get_velocity(self, param_list=None):
        ...
        if 'x' in param_list:
            vel['x'] = float(self._ask('x', '1 getnvel'))
        if 'y' in param_list:
            vel['y'] = float(self._ask('y', '2 getnvel'))
        if 'z' in param_list:
            vel['z'] = float(self._ask('z', '3 getnvel'))
        if 'phi' in param_list:
            vel['phi'] = float(self._ask('phi', '4 getnvel'))
    ...
    def set_velocity(self, param_dict):
        ...
        for axis_label in param_dict:
            ...
            if not(constr['vel_min'] <= desired_vel <= constr['vel_max']):
                ...
                self._write(axis_label, '{} {} setnvel'.format(desired_vel*self.unit_factor,constraints[axis_label]['ID']))
...
########################## internal methods ##################################
    ...
    def _write(self, axis, command):
        ...
        constraints = self.get_constraints()
        try:
            self._serial_connection_xyzphi.write(command + self._term_char_xyzphi)
    ...
    def _read_answer(self, axis):
        ...
        while still_reading == True:
            try:
                answer = answer + self._serial_connection_xyzphi.read()[:-2]
            except:
                still_reading = False
        return answer
    ...
    def _ask(self, axis, question):
        ...
        self._serial_connection_xyzphi.write(question+self._term_char_xyzphi)
    ...
    def _do_move_rel(self, axis, step):
        ...
        """
        constraints = self.get_constraints()
        if not (abs(constraints[axis]['pos_step']) < abs(step)/self.unit_factor):
            self.log.warning('Cannot make the movement of the axis "{0}"'
                             'since the step is too small! Ignore command!')
        else:
            #self._write('x','{} {} nr'.format(step,constraints[axis]['ID']))  THE RIGHT WAY TO DO IT!!!
            if axis == 'x':
                self._write('x', '{} 1 nr'.format(step))
            elif axis == 'y':
                self._write('y', '{} 2 nr'.format(step))
            elif axis == 'z':
                self._write('z', '{} 3 nr'.format(step))
            elif axis == 'phi':
                self._write('phi', '{} 4 nr'.format(step))
        return 0